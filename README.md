...
**Umbra** — это локальная десктоп-утилита для Windows и macOS, предназначенная для **анонимизации** юридических документов (`.docx`, `.pdf`, `.xlsx`, `.txt`, `.md`, а также `.doc` на Windows). Программа заменяет чувствительные данные на категорийные метки (например, `[ФИО_1]`, `[ОРГАНИЗАЦИЯ_2]`), а также умеет **деанонимизировать** их — подставлять реальные данные обратно в документ (например, при обработке ответа от ИИ).
