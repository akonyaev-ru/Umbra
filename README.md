...
Приложение защищено паролем, который запрашивается при первом запуске и каждые 30 дней.
Перед сборкой приложения вы можете задать или изменить свой пароль:
1. Перейдите в папку `src/license_tools/` и выполните команду:
   ```cmd
   python set_password.py
   ```
2. Введите ваш новый пароль. Скрипт безопасно запишет только SHA-256 хеш вашего пароля в код.
3. После изменения пароля обязательно пересоберите `Umbra.exe` по инструкции ниже.
